...
class _ScreenCalendarState extends State<HolidayScreem> {
    ...
    if (response.statusCode == 200) {
      ...
      if (list.length > 0) {
        ...
        for (var i = 0; i < list.length; i++) {
          HolidayListModel _HolidayListModel = HolidayListModel.fromJson(list[i]);
          listHoliday.add(_HolidayListModel);

          log.fine(" $i >> "+_HolidayListModel.startDate.toString());
          log.fine(int.parse(_HolidayListModel.startDate.toString().substring(0,2)).toString()+" , "+_HolidayListModel.startDate.toString().substring(3,5).toString() +" , "+_HolidayListModel.startDate.toString().substring(6,10).toString());

          _markedDateMap.add(
              DateTime(int.parse(_HolidayListModel.startDate.toString().substring(6,10)),int.parse(_HolidayListModel.startDate.toString().substring(3,5)),int.parse(_HolidayListModel.startDate.toString().substring(0,2))),
              new Event(
                date:  DateTime(int.parse(_HolidayListModel.startDate.toString().substring(6,10)),int.parse(_HolidayListModel.startDate.toString().substring(3,5)),int.parse(_HolidayListModel.startDate.toString().substring(0,2))),
  ...
  void refresh(DateTime date) {
    ...
    if (_markedDateMap.getEvents(new DateTime(date.year, date.month, date.day))
        .length>0) {
